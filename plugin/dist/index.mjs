    ...
    register(api) {
        ...
        api.on("session_end", async (event, ctx) => {
            ...
            if (process.platform === "darwin") {
                try {
                    execSync(`osascript -e 'display notification "Session complete (${event.messageCount} messages)" with title "OpenClaw MAS"'`, { timeout: 5000 });
                }
                catch {
                    // 静默失败
                }
...
function runQualityGate(filePath) {
    ...
    if (ext === ".go") {
        const result = spawnSync("gofmt", ["-l", filePath], { encoding: "utf8", timeout: 10000 });
