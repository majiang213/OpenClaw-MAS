    ...
    register(api) {
        ...
        api.on("before_tool_call", async (event, _ctx) => {
            ...
            if (toolName === "Bash" && /--no-verify/.test(cmd)) {
                return {
                    block: true,
                    blockReason: "🚫 --no-verify is blocked. Fix the underlying hook issue instead of bypassing it.",
                };
            }
            // 2. git-push-reminder
            if (toolName === "Bash" && /\bgit\s+push\b/.test(cmd)) {
                return {
                    requireApproval: {
                        title: "Git Push",
                        description: "You're about to push. Run `git diff HEAD~1` to review your changes first.",
                        severity: "info",
                        timeoutMs: 30000,
        ...
        api.on("after_tool_call", async (event, ctx) => {
            ...
            if (toolName === "Bash" && cmd) {
                ...
                appendLog("bash-commands.log", `[${new Date().toISOString()}] ${sanitized}`);
                appendLog("cost-tracker.log", `[${new Date().toISOString()}] tool=Bash durationMs=${durationMs ?? ""} command=${sanitized}`);
            }
            // 8. pr-created
        ...
        api.on("session_end", async (event, ctx) => {
            ...
            if (process.platform === "darwin") {
                try {
                    (0, node_child_process_1.execSync)(`osascript -e 'display notification "Session complete (${event.messageCount} messages)" with title "OpenClaw ECC"'`, { timeout: 5000 });
                }
                catch {
                    // 静默失败
                }
...
function getStagedFiles() {
    const result = (0, node_child_process_1.spawnSync)("git", ["diff", "--cached", "--name-only", "--diff-filter=ACMR"], {
        encoding: "utf8",
    });
    if (result.status !== 0)
        return [];
...
function getStagedFileContent(filePath) {
    const result = (0, node_child_process_1.spawnSync)("git", ["show", `:${filePath}`], { encoding: "utf8" });
    if (result.status !== 0)
        return null;
...
function findFileIssues(filePath) {
    ...
    if (!checkableExts.some((ext) => filePath.endsWith(ext)))
        return [];
...
function checkConsoleLog(filePath) {
    ...
    if (found.length > 0) {
        appendLog("console-log-warnings.log", `${new Date().toISOString()} ${filePath}: console.log at lines ${found.join(", ")}`);
    }
}
function runQualityGate(filePath) {
    ...
    if (ext === ".go") {
        const result = (0, node_child_process_1.spawnSync)("gofmt", ["-l", filePath], { encoding: "utf8", timeout: 10000 });
        if (result.stdout?.trim()) {
            appendLog("quality-gate.log", `${new Date().toISOString()} gofmt: ${filePath} needs formatting`);
        }
    }
    if (ext === ".py") {
        const result = (0, node_child_process_1.spawnSync)("python3", ["-m", "py_compile", filePath], {
            encoding: "utf8",
            timeout: 10000,
        });
        if (result.status !== 0) {
            appendLog("quality-gate.log", `${new Date().toISOString()} python syntax error: ${filePath} - ${result.stderr ?? ""}`);
        }
    }
}
function runBatchFormatCheck(files) {
    const tsSrcFiles = files.filter((f) => /\.(ts|tsx)$/.test(f) && fs.existsSync(f));
    if (tsSrcFiles.length === 0)
        return;
    const tsconfigPath = path.join(process.cwd(), "tsconfig.json");
    if (!fs.existsSync(tsconfigPath))
        return;
    const result = (0, node_child_process_1.spawnSync)("npx", ["tsc", "--noEmit", "--skipLibCheck"], {
        cwd: process.cwd(),
        encoding: "utf8",
        timeout: 60000,
    });
